...
class Spectrum:
    ...
    def run_calibration(self, order, method='ls', bounds_pars=None, verbose=False, n=1000, delta=0.1):
        ...
        if method == 'ls':
            popt, pcov = curve_fit(self.model.func, self.dataset.px, self.dataset.lines)
            ...
            chisq = ((self.dataset.lines - self.calibration(self.dataset.px))**2 / self.dataset.errpx**2).sum()
            dof = len(self.dataset.lines) - len(popt)

            fit_status = FitStatus(popt, pcov, chisq, dof, order=self.calibration.order)
            return fit_status
            ...
            self.sampler = MHsampler(self.dataset.px, self.dataset.lines, self.dataset.errpx,
            ...
            def log_likelihood(x, y, errx, x_hat, theta):
                ...
                term1 = np.array([np.exp(-0.5 * (x - z)**2 / errx**2) / abs(model.derivative(z)) for z in zeros])
            ...
            samples, x_hat_s, s_orig, x_orig, acc, rej = self.sampler.run(verbose=verbose)
    ...
    def show_calibration(self, exclude=None, **kwargs):
        ...
        ax1.grid()
        errs = abs(self.calibration.derivative(self.dataset.px)) * self.dataset.errpx
        ax1.scatter(self.dataset.px, (self.dataset.lines - self.calibration(self.dataset.px)) / errs,
                   marker='+', color='black')
        ax1.set_xlim(0, x[-1])
        ...
        ax.plot(line, self.l_int, linestyle='solid', color='black', linewidth=0.5)
        for lam in self.dataset.lines:
            if lam == self.dataset.lines.min() or lam == self.dataset.lines.max():
                ax.axvline(lam, ymin=0, ymax=1, linewidth=0.5, color='red', linestyle='dotted')
            else:
                ax.axvline(lam, ymin=0, ymax=1, linewidth=0.5, color='navy', linestyle='dashed')
                ax.text(lam, self.l_int.max()/2, '{:s}'.format(self.dataset.names[lam]),
        ...
        ax.grid()
        line = self.calibration(x)
        ax.plot(line, self.s_int, linestyle='solid', color='black', linewidth=0.5)
        for lam in self.dataset.lines:
            if lam == self.dataset.lines.min() or lam == self.dataset.lines.max():
                ax.axvline(lam, ymin=0, ymax=1, linewidth=0.5, color='red', linestyle='dotted')
            elif exclude is not None and self.dataset.names[lam] not in exclude:
                ax.axvline(lam, ymin=0, ymax=1, linewidth=0.5, color='navy', linestyle='dashed')
                ax.text(lam, (self.s_int.max() + self.s_int.min()) / 2, '{:s}'.format(self.dataset.names[lam]),
                        rotation=90, verticalalignment='center', horizontalalignment='left',
                        size=7.5, color='navy')
        ax.set_xlim(line.min(), line.max())
        ax.set_xlabel(r'$\lambda$ [$\mathring{A}$]')
    ...
    def compare(self, spectrum):
        ...
        if len(ref_calibration) == len(cmp_calibration) and np.all(self.calibration == eq_spectrum):
        ...
        for _, pt in enumerate(ref_calibration):
            ...
            l_approx = fs.Linear((upp_y - low_y) / (upp_x - low_x),
                                 low_y - (upp_y - low_y) / (upp_x - low_x) * low_x)
